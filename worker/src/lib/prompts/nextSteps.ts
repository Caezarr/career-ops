/**
 * Next-steps generator prompt + tool schema (server-side mirror of
 * `src-tauri/src/ai/prompts/next_steps.rs`).
 */

export const NEXT_STEPS_SYSTEM = `You are a senior career strategist coaching candidates targeting top-tier firms (Goldman Sachs, McKinsey, BCG, Stripe, OpenAI tier).
...
Role: ${role}
Current stage: ${stage}
</application>${jdBlock}${cvBlock}

Produce 3-5 concrete next-step actions for THIS application. Use the \`next_steps\` tool — output the structured call only.`;
}
  ...
  properties: {
    steps: {
      ...
      items: {
        type: "string",
        description:
          "A single concrete next-step action under 140 characters. Lead with a verb, no filler.",
      },
    },
  },
  required: ["steps"],
